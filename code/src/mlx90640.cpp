...
namespace r2d2::thermal_camera {
    ...
    mlx90640_c::mlx90640_c(i2c::i2c_bus_c &bus, const uint16_t refresh_rate,
        ...
        : mlx90640_processor_c(bus, address) {
        hwlib::wait_ms(1000);
    ...
    uint16_t mlx90640_c::get_refresh_rate() const {
        uint16_t rate = read_register(INTERNAL_CONTROL_REGISTER);
        rate >>= 7 & 7;
        return 1 << (rate - 1);
    }
    ...
    void mlx90640_c::set_refresh_rate(uint16_t refresh_rate) const {
        ...
        uint16_t data = read_register(INTERNAL_CONTROL_REGISTER);
        for (uint8_t i = 0; i < 3; i++) {
            toggle_nth_bit(data, i + 7, refresh_rate >> i & 1);
        }
        write_register(INTERNAL_CONTROL_REGISTER, data);
    }

    bool mlx90640_c::frame_available() const {
        uint16_t data = read_register(INTERNAL_STATUS_REGISTER);
        ...
        write_register(INTERNAL_STATUS_REGISTER, data);
